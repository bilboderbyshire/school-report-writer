...
class TemplateEngine:
    ...
    def edit_piece(self, section, piece_id, new_piece):
        self.copy_of_pieces[section][piece_id].piece_text = new_piece

    def add_piece(self, section) -> str:
        new_id = f"@{self.new_id_num_cache[section]}"
        self.new_id_num_cache[section] += 1

        self.copy_of_pieces[section][new_id] = IndividualPiece(NewPieceRecord(new_id, section))

        return new_id

    def delete_piece(self, section, piece_id):
        self.copy_of_pieces[section].pop(piece_id)
